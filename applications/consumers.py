...
class ApplicationConsumer(AsyncWebsocketConsumer):
    async def connect(self):
        await self.channel_layer.group_add("applications", self.channel_name)
    ...
    async def disconnect(self, close_code):
        await self.channel_layer.group_discard("applications", self.channel_name)
        host_group = self._get_host_group()
        if host_group:
    ...
    async def app_created(self, event):
